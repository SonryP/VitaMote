...
public class CusMap extends Activity {
    ...
    private void loadMapping() {
        try {
            List<Integer> loadedKeys = loadCM(this);
            if (loadedKeys == null) {
                throw new IOException("Failed to load custom mapping from file.");
            }
            for (int i = 0; i < spinners.length && i < loadedKeys.size(); i++) {
                Integer keyCode = loadedKeys.get(i);
                if (keyCode != null) {
